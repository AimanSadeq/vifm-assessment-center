...
src/
  app/
    ...
    ara/
      personal/start/                  # Free Personal AI Readiness Snapshot entry (anonymous)
      personal/results/[token]/        # Bilingual results page — factor scores + course recommendations + PDF download
      consultant/personal-deep-dive/new/  # Paid 48-item deep-dive issuance (requires admin/consultant role)
      consultant/                      # Consultant dashboard with personal-snapshot-activity panel (last 30d, snapshot vs deep-dive)
      consultant/assessments/[id]/     # Org assessment detail — adds Workforce Readiness rollup card on Phase 2 tab when departmental-plus-personal
    ...
    api/
      ...
      ara/reports/[assessmentId]/pdf/          # Bilingual EN/AR/side-by-side ARA PDF (Puppeteer) — includes Workforce Readiness section when departmental-plus-personal
  ...
  lib/
    ...
    ara/                  # ARA-specific helpers — auth-guards, email (3 ARA templates), respondent-access, scoring, distortion, year-on-year, peer-benchmarks, regulatory engine, workforce-readiness rollup (departmental-plus-personal)
...
supabase/
  migrations/
    ...
    00002_seed_competencies.sql               # 4 domains, 9 clusters, 41 competencies
    ...
    00026_ara_individual_seed.sql             # 16 self-assessment items on v1.1 (4 per factor) — free-snapshot baseline
...
- **Competency:** A measurable behavioral dimension. VIFM framework: 4 domains, 9 clusters, 41 competencies with 249 behavioral indicators (positive/negative), 114 development tips, 5 tags, and 3 Q&A questions per competency
...
- **PDF reports:** Puppeteer (Arabic shaping + landscape bilingual side-by-side layout). Keep React-PDF for AC candidate reports. Endpoint: `/api/ara/reports/[assessmentId]/pdf?language=en|ar|bilingual`. **Chromium launch goes through the shared `launchPdfBrowser()` in [src/lib/reports/pdf-browser.ts](src/lib/reports/pdf-browser.ts)** — bundled puppeteer Chromium in dev, `@sparticuz/chromium` in production. This is deliberate: Render persists `node_modules` but NOT puppeteer's HOME Chromium cache (`~/.cache/puppeteer`), so plain `puppeteer.launch()` 500s in prod with "Could not find Chrome (ver. 147.x)". `@sparticuz/chromium@147` ships its Chromium binary inside `node_modules` (version-matched), so it survives the build→runtime hand-off with no install step or dashboard change. Selection: `@sparticuz` when `RENDER`/`NODE_ENV=production`/`USE_SPARTICUZ_CHROMIUM=1`. Arabic shaping is unaffected — every Arabic PDF loads the Noto Naskh webfont and waits for `fonts.ready`, so HarfBuzz shapes from the loaded font, not the Chromium build's system fonts. Every Puppeteer route (ARA report/personal/methodology, Reflect participant/cohort/framework, Pre-Hire candidate PDF, technical programme/cohort) routes through this helper.
...
- **Free-personal-snapshot norm group accumulation:** percentile claims like "you scored at the X% percentile of GCC respondents" require ~200-500 completed Personal AI Readiness Snapshots in the DB. Passive — accumulates as people take the free snapshot. No action needed until the volume's there.
...
- **Personal AI-readiness report (unified renderer, 2026-09-02):** ONE HTML renderer for both languages, `src/lib/reports/personal-report-html.ts`, printed through Chromium via `src/lib/reports/personal-report-build.ts` (`buildPersonalReportPdf`), which every caller shares - the token route `/api/ara/personal/[token]/pdf` and the public sample `/samples/arc-report` (a real render of a sandbox fixture, "Abdullah Alanazi" at Ufuq Digital Authority, `?lang=ar` for Arabic). Replaced the React-PDF EN + Puppeteer AR pair that had drifted apart. Layout: executive summary, four factor cards with self-rated vs demonstrated sub-scores (deep-dive), strengths, priorities, 30/60/90, comparison against unit and organisation (departmental-plus-personal only), training, GROW manager guide, fact sheet. Bilingual copy (12 stage x factor blurbs, next-steps, legend) lives in `src/lib/reports/personal-report-copy.ts`.
...
### ARC interactive dashboard (live, any level) + optional segments
`buildDashboardTree(rootAssessmentId)` in [src/lib/ara/dashboard-tree.ts](src/lib/ara/dashboard-tree.ts) turns the real engine into one tree - organisation (enterprise rollup) -> divisions (pooled) -> departments (leaf `ara_pillar_scores`) -> people (`computeWorkforceReadiness`, four factors) - and [src/components/ara/arc-dashboard.tsx](src/components/ara/arc-dashboard.tsx) renders it as a bilingual drill-down (breadcrumbs, tiles, tabs Overview / Units by pillar / People / Segments / Training, each only when it has data). Mount points: `/ara/consultant/assessments/[id]/dashboard` (consultant, ownership-gated, PDF links per node, "Dashboard" button on the assessment page) and the public sample `/samples/arc-dashboard` (Ufuq enterprise fixture). Every number is the number the PDF reports print; the component never re-derives a score, band or level.
- **Segments are optional** (client decision 2026-09-02): migration 00202 adds `ara_respondents.demographics jsonb`; vocabulary in [src/lib/constants/ara-demographics.ts](src/lib/constants/ara-demographics.ts) (grade / tenure / gender / nationality as national-vs-expatriate). Individual-layer respondents get an "About you (optional)" card on the form (`setAraRespondentDemographics`, "Prefer not to say" on every question). The dashboard shows aggregates only and drops any slice under `ARA_SEGMENT_MIN_N` (3); demographics never appear per person, in a report, or in an export.
...
- **Rollup + deliverables:** `computeAgenticReadiness(assessmentId)` ([src/lib/ara/agentic-readiness.ts](src/lib/ara/agentic-readiness.ts)) returns cohort mean per dimension + per-respondent breakdown + cohort overall (target = 4; null when no agentic answers yet) — mirrors the departmental-plus-personal workforce rollup. Surfaces as a rollup card on the consultant assessment detail and an "Agentic-AI Readiness" section in the bilingual PDF report (cohort overall + six per-dimension rows; renders only when the layer is on and respondents have answered).
...
- **Tag tables:** `vifm_course_competency_tags` (course → AC's 41 behavioural competencies with relevance weight 1-3 + rationale + source 'manual'/'ai_proposed'/'ai_accepted') and `vifm_course_pillar_tags` (course → ARA's 8 pillars, same shape). Two-axis tagging because the PDF's own "Target Competencies" block is topical (e.g. "Bookkeeping Automation") while AC competencies are behavioural (e.g. "Strategic Thinking") — different ontologies serving different recommender contexts.
...
- `recommendCoursesForIndividualSnapshot({ factorScores })` — all three personal modes personal readiness → maps each below-target factor to AC competency names from `ARA_INDIVIDUAL_FACTORS`, looks up courses tagged to those competencies.
...
- **ARA Phase 2 tab** (`/ara/consultant/assessments/[id]`): "Capability-building plan" card listing courses ranked by per-pillar maturity gap. When the departmental-plus-personal layer is on, also shows a "Workforce training plan" panel using the cohort factor scores.
...
The letters A/B/C were internal shorthand and have been retired in favour of
names that say what each mode is (client instruction 2026-09-01). None of this
vocabulary was ever client-facing.

| Mode | Stage | Tier | Items | Issuance | URL |
|---|---|---|---|---|---|
| Free personal snapshot | `individual` | `snapshot` | 24 (6/factor) | Anonymous self-served | `/ara/personal/start` |
| Paid personal deep-dive | `individual` | `deep_dive` | 48 (12/factor) | Consultant-issued | `/ara/consultant/personal-deep-dive/new` |
| Departmental plus personal | dept/division/enterprise + `include_individual_layer=true` | `snapshot` or `deep_dive` | 24 or 48 | Wizard toggle on `/ara/consultant/assessments/new` | (alongside org pillar items) |

All three modes share infrastructure: same `ara_questions` table (distinguished by `individual_factor_id` + `tier`), same `ara_respondents` flow (departmental-plus-personal optionally `individual_only=true` to skip pillar questions), same results page (`/ara/personal/results/[token]` — gated to individual-stage OR include_individual_layer assessments) and same PDF endpoint (`/api/ara/personal/[token]/pdf`).

### Departmental-plus-personal consultant deliverables
...
The **Foundations layer** of the layered measurement model (the bottom of the spine the layered-model note above describes): cognitive ability + Big-Five personality **PREDICT** the behavioural 41 — the honest answer to "where does psychometric testing fit, and how do we cover High-Potential / Succession?". It is the propensity tier (weakest evidence; validate before high-stakes use), distinct from *enables* (technical/language attainments) and *manifests* (the AC/CBI/360 score itself). Tier 1 is **INDICATIVE** — classical scoring, no local norms, no IRT calibration yet, and **issues no credential**. Full rationale + the HiPo/Succession payoff in [docs/psychometrics-proposal.md](docs/psychometrics-proposal.md).
...
- Runner: **`/ac/cognitive`** (optional `?candidateId=&engagementId=` to bind an admin-run test to a candidate record). (The standalone OCEAN/Big-Five *personality* runner once documented at `/ac/psychometrics` is **retired** - Persona is the behavioural self-report instrument now; `/ac/psychometrics` 404s and nothing links to it. The SME bank console at `/admin/psychometrics` still manages both cognitive + personality item banks.)
- API: **`/api/ac/cognitive`** — `{action:"start", lang}` inserts a session + returns the answer-key-stripped test; `{action:"score", session_id, answers}` reloads the session, grades, writes `psy_results` + `psy_item_responses`, marks consumed. Tolerant: returns a helpful "apply migration 00065" error when the tables are absent. (There is no `/api/ac/psychometrics` route.)
