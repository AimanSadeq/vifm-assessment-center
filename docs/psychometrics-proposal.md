...
*Working draft for internal discussion. Owner: VIFM. Status: **Tier 1 shipped + Tier 2 calibration engine in place.** Tier 1 (indicative) — cognitive + Big-Five (Mini-IPIP) runner at `/ac/psychometrics`, secure server-held key, admin-only results, Foundations→competency `predicts` bridge (00065 + 00066). Tier 2 substrate (migration 00067 + `src/lib/psychometrics/calibration.ts`) — norms table + IRT columns + reliability (Cronbach's α) + raw→percentile/sten standardization + the indicative↔calibrated tier gate, all wired so the runner becomes norm-referenced the moment a norm group is loaded (clean Tier-1 fallback). Tier 2 engineering is **complete**: the SME item-bank console at `/admin/psychometrics` (three-gate readiness, AI-draft, approve/retire/edit review lifecycle), per-scale Cronbach's α from the response log, bank-driven test assembly from approved items, a pilot norm-loader, the public-domain **IPIP-50** longer validated form (one-click seed into the bank), and a **professional norm-referenced report PDF** (per-scale percentile + sten band + validity statement; verified live on both the indicative and calibrated paths). **Remaining for full Tier 2 — non-code only:** accumulate the norm sample, run the criterion-validity + fairness study, and obtain a psychometrician's sign-off. (Optional future engineering: an IPIP-120 / IRT-CAT adaptive form.) Note Tier 2 yields scores + a defensible report, **not** a pass/fail credential. See the "VIFM Psychometrics" section in CLAUDE.md.*
