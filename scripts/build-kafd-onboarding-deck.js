/**
 * Build the KAFD client ONBOARDING deck (.pptx) for the AI Readiness Compass
 ...
 *
 * Run:  npm install --no-save pptxgenjs && node scripts/build-kafd-onboarding-deck.js
 */
...
const FOOTER = "VIFM AI Readiness Compass   |   KAFD Onboarding";

const pres = new pptxgen();
pres.layout = "LAYOUT_16x9";       // 10 x 5.625 in
pres.author = "VIFM";
pres.company = "Virginia Institute of Finance and Management";
pres.title = "AI Readiness Compass - KAFD Onboarding";
...
// ════════════════════════════════════════════════════════════════
{
  ...
  eyebrow(s, "[  CLIENT ONBOARDING   |   KING ABDULLAH FINANCIAL DISTRICT  ]", 0.35, 0.32, C.accent);
  ...
  s.addText("Your access, and how it works - a step-by-step guide for KAFD delegates.", { x: 0.37, y: 3.7, w: 7.6, h: 0.6, fontSize: 15, italic: true, color: C.light, fontFace: FONT });
  // chips
  const chips = ["20 personal access codes", "~10 minutes", "Bilingual EN / AR", "Private to you"];
  let cx = 0.37;
  chips.forEach((t) => {
...
{
  const s = pres.addSlide();
  contentHeader(s, "[  YOUR ACCESS  ]", "What KAFD has been given");
  // big stat card
  ...
  s.addText("20", { x: 0.35, y: 1.55, w: 3.4, h: 1.5, fontSize: 96, bold: true, color: C.accent, fontFace: FONT, align: "center", valign: "middle" });
  s.addText("personal access codes", { x: 0.35, y: 3.05, w: 3.4, h: 0.5, fontSize: 16, bold: true, color: C.white, fontFace: FONT, align: "center" });
  s.addText("One per delegate.\nEach single-use.", { x: 0.35, y: 3.6, w: 3.4, h: 0.9, fontSize: 12.5, color: C.light, fontFace: FONT, align: "center", wrap: true });
  ...
  const rights = [
    ...
    ["VIFM handles delivery", "We send the invitations and resend on request. KAFD does not need to manage codes."],
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const rows = [
    ...
    ["Private to you", "Your individual report is yours. KAFD and VIFM see completion and an anonymised group picture - not your answers."],
  ];
  let yy = 1.35;
  rows.forEach((r, i) => {
...
// ════════════════════════════════════════════════════════════════
// 11 - For the KAFD coordinator
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  contentHeader(s, "[  FOR THE KAFD COORDINATOR  ]", "Running this for your team");
  const items = [
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const steps = [
    ["Now", "Codes issued", "KAFD's 20 access codes are ready to send."],
  ...
  s.addText("Timeline is indicative and flexes to KAFD's schedule.", { x: 0.4, y: 4.6, w: 9.2, h: 0.4, fontSize: 11.5, italic: true, color: C.navy, fontFace: FONT, align: "center" });
  footer(s, 13);
}
...
pres.writeFile({ fileName: "KAFD-AI-Readiness-Compass-Onboarding.pptx" }).then((fn) => {
