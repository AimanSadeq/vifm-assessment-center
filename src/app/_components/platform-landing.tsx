...
// Jewel-per-service palette: each service carries its own hue. The .launcher-card
// tone-<hue> class supplies the matching --tone (tinted surface, gradient icon
// chip, corner glyph, hover glow); this map gives the resting border colour.
...
const VARIANTS: Record<Lang, Partial<Record<ServiceKey, Partial<Record<Pillar, Variant>>>>> = {
  en: {
    ...
    cognitive: {
      acquire: {
        tagline: "Reasoning aptitude · selection", badge: "For Selection",
      ...
      manage: {
        tagline: "Reasoning aptitude · development", badge: "For Development",
  ...
  ar: {
    ...
    cognitive: {
      acquire: {
        tagline: "القدرة على الاستدلال · للاختيار", badge: "للاختيار",
      ...
      manage: {
        tagline: "القدرة على الاستدلال · للتطوير", badge: "للتطوير",
...
const T = {
  en: {
    ...
    services: {
      ...
      cognitive: { tagline: "Reasoning aptitude", name: "Mentium®", description: "Indicative numerical, verbal, inductive and deductive reasoning - a foundational read on aptitude. Server-scored, admin-run and bilingual.", tooltip: "Best for a foundational read on reasoning and aptitude." },
  ...
  ar: {
    ...
    services: {
      ...
      cognitive: { tagline: "القدرة على الاستدلال", name: "مينتيوم", description: "مقاييس استرشادية للاستدلال العددي واللفظي والمجرّد - قراءة تأسيسية للقدرات. تُصحَّح على الخادم، يُجريها المسؤول، وثنائية اللغة.", tooltip: "الأنسب لقراءة تأسيسية للاستدلال والقدرات." },
...
export function PlatformLanding() {
  ...
  // light/accent blue (#5391D5) = For selection (Talent Acquisition), dark blue
  ...
  const renderServiceCard = (svc: (typeof SERVICES)[number]) => {
    ...
    // Jewel-per-service (modern): every service gets its OWN hue. The
    // .launcher-card tone-<hue> class dresses the tinted surface, gradient icon
    // chip, corner glyph and tagline; the resting border is a soft tint of the
