...
import { countExpiredFluentResults, purgeFluentResults } from "./actions";
import { RETENTION_MONTHS, PURGE_CONFIRMATION } from "./constants";

export const dynamic = "force-dynamic";
export const metadata = { title: "Fluent® retention · VIFM" };

export default async function FluentRetentionPage() {
  const caller = await getCurrentCaller();
  if (!caller || caller.role !== "admin") return notFound();

  const { total, expired } = await countExpiredFluentResults();

  const purgeAction = async (fd: FormData) => {
    "use server";
    await purgeFluentResults(fd);
  };
      ...
      <Card className="border-destructive/50">
        <CardHeader>
          ...
          <CardDescription>
            ...
            (and their calibration score-runs). Voucher redemption records are kept (their result link is cleared).
          </CardDescription>
        </CardHeader>
        <CardContent>
