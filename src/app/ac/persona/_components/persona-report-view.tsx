...
import { personaBand, personaBandLabel, PERSONA_BAND_TW, gapToneHex } from "@/lib/scoring/persona-bands";
                ...
                <div key={r.name}>
                  ...
                  <div className="mt-1 h-2 w-full rounded-full bg-slate-100">
                    {/* CAL-PER-404: when a role target exists, colour the bar by
                        ...
                        Inline hex (not a Tailwind class) so it always compiles
                        and matches the PDF/AR renderers exactly. */}
                    <div
                      className="h-2 rounded-full"
                      style={{ width: `${(r.score / 5) * 100}%`, background: r.target != null ? gapToneHex(r.score, r.target) : "#5391D5" }}
                    />
                  </div>
