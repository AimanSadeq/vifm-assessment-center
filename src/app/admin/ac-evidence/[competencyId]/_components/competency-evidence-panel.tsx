...
import { suggestCompetencyEvidence, saveCompetencyEvidence } from "@/lib/ac/evidence-actions";
import type { ValidationEvidence, EvidenceConfidence } from "@/types/evidence";

/**
 * Per-competency validation-evidence editor for the Assessment Center.
 * Plain English (the admin portal is not localized). Mirrors the ARC
 * ValidationEvidencePanel: generate AI suggestion → review (accept /
 * edit / reject) → only verified/edited anchors reach client surfaces.
 */
...
export function CompetencyEvidencePanel({ competencyId, reviewerEmail, initialEvidence }: Props) {
  ...
  function generate() {
    ...
    startTransition(async () => {
      const r = await suggestCompetencyEvidence(competencyId);
      if (!r.ok) {
        setError(r.error);
        return;
      }
  ...
  function persist(next: ValidationEvidence) {
    setError(null);
    startTransition(async () => {
      const r = await saveCompetencyEvidence(competencyId, next, reviewerEmail);
      if (!r.ok) {
        setError(r.error);
        return;
      }
    ...
    return (
      <PanelShell>
        <p className="text-sm text-muted-foreground">
          ...
          assessment-centre bibliography (you verify before it reaches any client), or add
          anchors manually.
        </p>
