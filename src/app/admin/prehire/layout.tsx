 ...
 */
export default function PrehireLayout({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex min-h-screen flex-col bg-[#F5F7FA]" dir="ltr">
      {/* Branded top bar */}
        ...
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3">
          <div className="flex items-center gap-3">
            ...
            <span className="inline-flex items-center gap-1.5 text-sm font-semibold text-white">
              <UserSearch className="h-4 w-4 text-[#FDA4AF]" /> Pre-Hire
            </span>
          </div>
          <nav className="flex items-center gap-1.5 text-xs font-medium">
            ...
            >
              <ClipboardList className="h-3.5 w-3.5" /> Requisitions
            </Link>
            <Link
              href="/courses"
            ...
            >
              <LayoutGrid className="h-3.5 w-3.5" /> All services
            </Link>
          </nav>
        </div>
      </header>
      ...
      <footer className="border-t bg-card/50">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-2 px-6 py-4 sm:flex-row sm:items-center">
          <div className="text-xs text-muted-foreground">
            ...
            VIFM Pre-Hire · Confidential — for VIFM and engaged clients only.
          </div>
