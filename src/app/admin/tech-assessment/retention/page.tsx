...
import { countExpiredTechnical, purgeTechnical } from "./actions";
import { RETENTION_MONTHS, PURGE_CONFIRMATION } from "./constants";

export const dynamic = "force-dynamic";
export const metadata = { title: "Technical retention · VIFM" };

export default async function TechnicalRetentionPage() {
  const caller = await getCurrentCaller();
  if (!caller || caller.role !== "admin") return notFound();

  const { total, expired } = await countExpiredTechnical();

  const purgeAction = async (fd: FormData) => {
    "use server";
    await purgeTechnical(fd);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        ...
        results, server-held test sessions and performance-based sandbox sessions past that window to stay
        compliant. This is irreversible.
      </p>

      <div className="grid gap-4 sm:grid-cols-2 mb-6">
        <Card>
          <CardHeader className="pb-2">
            <CardDescription>Results + sessions stored</CardDescription>
          </CardHeader>
          <CardContent>
      ...
      <Card className="border-destructive/50">
        <CardHeader>
          <CardTitle className="text-base text-destructive flex items-center gap-2">
            <AlertTriangle className="h-4 w-4" /> Purge expired records
          </CardTitle>
          <CardDescription>
            Permanently deletes the {expired} record{expired === 1 ? "" : "s"} older than {RETENTION_MONTHS / 12} years
            across technical results, test sessions and sandbox sessions (sandbox responses cascade). Issued credentials are kept.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form action={purgeAction} className="flex items-end gap-3">
            ...
            <Button type="submit" variant="destructive" disabled={expired === 0}>
              Purge {expired} record{expired === 1 ? "" : "s"}
            </Button>
          </form>
        </CardContent>
      </Card>
