...
function renderProposalDocAr(
  ...
  const dataResidencyStatementAr = (r: DataResidency): string => {
    switch (r) {
      ...
      default:
        return "تُستضاف جميع بيانات المرشحين والتقييم على سحابة VIFM المُدارة، مع إتاحة سيادة البيانات داخل الدولة (السعودية أو الإمارات) عند الطلب مقابل تكلفة إضافية يُتفق عليها مع العميل.";
    }
  };
