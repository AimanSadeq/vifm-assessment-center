...
export type PsyReportScale = {
  ...
  name: string;
  predicts: string[];
  raw: number;        // cognitive: % correct
  rawLabel: string;   // e.g. "62%"
...
export function PsychometricReport({ data, provisional }: { data: PsyReportData; provisional?: boolean }) {
      ...
      <Page size="A4" style={s.page} wrap>
        {provisional && (
          <View style={{ borderWidth: 1, borderColor: "#f59e0b", backgroundColor: "#fffbeb", borderRadius: 4, padding: 8, marginBottom: 10 }}>
            ...
            <Text style={{ fontSize: 8, color: "#92400e", marginTop: 2 }}>
              This sitting was generated live and its items have not been reviewed and approved by a subject-matter expert. Treat these results as indicative until the content review is complete.
            </Text>
          </View>
        )}
        {/* Hero */}
        <View style={s.hero}>
          ...
          <View style={s.heroPillRow}>
            ...
            <Text style={s.heroPill}>Logica®</Text>
          </View>
        </View>
        ...
        <Text style={s.sectionTitle}>Reasoning subtests</Text>
      ...
      <Page size="A4" style={s.page} wrap>
        ...
        <View style={s.panel}>
          ...
          <Text style={s.panelBody}>
            This instrument sits in the Foundations layer of the VIFM measurement model: reasoning ability is a
        ...
        <View style={s.disclaimerBox}>
          ...
          <Text style={s.disclaimerBody}>
            Logica is estimated from numerical, verbal, inductive and deductive reasoning items, scored as % correct per subtest with a general (g) composite.{" "}
            {calibrated
